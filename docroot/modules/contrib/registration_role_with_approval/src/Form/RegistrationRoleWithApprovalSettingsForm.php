...
class RegistrationRoleWithApprovalSettingsForm extends ConfigFormBase
{
  ...
  public function buildForm(array $form, FormStateInterface $form_state)
  {
    ...
    foreach ($system_roles as $system_role) {
      ...
      if ($role_id != '0' && $role_id != 'authenticated') {
        $form['profile_roles'][$system_role->id()] = array(
          '#type' => 'checkbox',
          '#title' => t($system_role->label()),
          '#default_value' => $profile_roles[$system_role->id()]['default'],
        );
        $form['profile_roles'][$system_role->id() . "needs_approval"] = array(
          '#type' => 'checkbox',
  ...
  public function submitForm(array &$form, FormStateInterface $form_state)
  {
    ...
    $config = $this->configFactory()->getEditable('registration_role_with_approval.settings');
    $config->set('profile_roles', $config_list)
